...
export const getComments = async (req: Request, res: Response):Promise<Response> => {
  ...
  let result = await pool.query('SELECT comments.*, users.email, stories.*, photos.photo_url FROM comments JOIN photos ON comments.story_id = photos.story_id JOIN users ON comments.user_id = users.id JOIN stories ON comments.story_id = stories.id ORDER BY date_comment DESC LIMIT $1 OFFSET $2', [limit, offset])
...
export const addComment = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (!comment) {
    return res.json({ error: 'comment required' })
  }

  const addedComment = await pool.query('INSERT INTO comments (comment, story_id, user_id) VALUES ($1, $2, $3) RETURNING *', [comment, storyId, userId])
