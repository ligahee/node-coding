...
export const getStories = async (req: Request, res: Response):Promise<Response> => {
  ...
  let result = await pool.query('SELECT stories.*, users.email, users.username FROM stories JOIN users ON stories.user_id = users.id ORDER BY date_added DESC LIMIT $1 OFFSET $2', [limit, offset])
...
export const getUserStories = async (req: Request, res: Response): Promise<Response> => {
  ...
  let result = await pool.query('SELECT stories.*, users.email, users.username FROM stories JOIN users ON stories.user_id = users.id WHERE users.id=$1 ORDER BY date_added DESC LIMIT $2 OFFSET $3', [req.params.id, limit, offset])
...
export const getComments = async (req: Request, res: Response):Promise<Response> => {
  ...
  let limit: number = Number(req.query.limit) || DEFAULT_LIMIT
  let offset: number = (page - 1) * limit
  let total = await pool.query('SELECT count(*) FROM comments')
  let result = await pool.query('SELECT comments.*, users.email, users.username, stories.* FROM photos JOIN users ON photos.user_id = users.id JOIN stories ON photos.story_id = stories.id ORDER BY date_comment DESC LIMIT $1 OFFSET $2', [limit, offset])

  let pages = Math.ceil(Number(total.rows[0].count) / limit)

  return res.status(200).json({
    total: {comments: total.rows[0].count, pages}, 
    page,
    rows: result.rows 
  });
}
...
export const getComment = async (req: Request, res: Response): Promise<Response> => {
  const result = await pool.query('SELECT * FROM comments WHERE comments.story_id=$1', [req.params.id])
  if (result.rowCount === 0) return response.sendStatus(404)
